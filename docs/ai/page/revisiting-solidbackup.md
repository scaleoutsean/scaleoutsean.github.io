# SolidBackup for SolidFire Data Protection in Hybrid Cloud
...
- [First attempt](#first-attempt)
  - [Backup and Restore in KVM environments](#backup-and-restore-in-kvm-environments)
  - [Data Migration](#data-migration)
  - [Do we really want DIY approaches to protect our data](#do-we-really-want-diy-approaches-to-protect-our-data)
- [Second attempt](#second-attempt)
  - [Create configuration and keep Src & Dst (Clone) volumes in sync](#create-configuration-and-keep-src--dst-clone-volumes-in-sync)
  - [Backup to S3](#backup-to-s3)
  - [Restore from S3](#restore-from-s3)
- [Getting there (aka "Digital Transformation")](#getting-there-aka-digital-transformation)
    - [`.snapshot`, but on SolidFire](#snapshot-but-on-solidfire)
- [Next Steps](#next-steps)
- [Conclusion](#conclusion)
- [Update (May 30, 2021)](#update-may-30-2021)
  - [SolidSync: PowerShell script that syncs Source to Target volumes](#solidsync-powershell-script-that-syncs-source-to-target-volumes)
  - [SolidBackup: stand-alone PowerShell script that does the following](#solidbackup-stand-alone-powershell-script-that-does-the-following)
  - [Rinse & reapeat](#rinse--reapeat)
  - [Why not one big script that does it all](#why-not-one-big-script-that-does-it-all)
- [Demo](#demo)
...
In [previous](/2021/04/22/solidfire-kvm-duplicati-and-backup-to-s3.html) posts I mentioned the idea for SolidBackup, a simple Backup & Restore solution possibly needed by Docker/K8s administrators who wanted a basic way to protect their data on SolidFire systems.
...
![SolidBackup Alpha in Python](/assets/images/solid-backup-prototype.gif)

(I have a "manual" walk-through (each step) [on YouTube](https://www.youtube.com/watch?v=bvI7pgXKh6w), but it's 23 minutes long.)

Before I finished that utility I discovered there was an app for that exact use case (simple container volume backup) and that exact audience (DIY/DevOps) - Velero - which works with NetApp Trident and SolidFire (see my [Velero-with-SolidFire post here](/2021/02/08/use-velero-with-netapp-solidfire-and-trident-csi), so I stopped working on SolidBackup.
...
- SolidFire CLI (Python) + Duplicati ([8 min video of Duplicati + KVM + SolidFire](https://youtu.be/wP8nAgFo8og) it was very similar to last year's attempt with Duplicacy so I didn't spend time on trying to automate it - I merely worked through the same steps
...
![Step one in solidbackup - solidsync](/assets/images/revisiting-solidbackup-01.png)
...
![Step two - mount Clone volume](/assets/images/revisiting-solidbackup-02.png)
...
![Step three - backup](/assets/images/revisiting-solidbackup-03.png)
...
![Restic backup in StorageGRID](/assets/images/revisiting-solidbackup-04.png)
...
![Step three - raw device backup](/assets/images/revisiting-solidbackup-05.png)
...
![solidbackup - view snapshots](/assets/images/revisiting-solidbackup-06.png)
...
![solidbackup - view snapshots](/assets/images/revisiting-solidbackup-07.png)
...
![solidbackup - view snapshots](/assets/images/revisiting-solidbackup-08.png)
...
![solidbackup - view snapshots](/assets/images/revisiting-solidbackup-09.png)
...
To those who contemplate automating some of their SolidFire workflows and are good at both PowerShell and Python, I would recommend to first consider [SolidFire SDK for Python](https://pypi.org/project/solidfire-sdk-python/) and if that's not an option then [SolidFire Tools for PowerShell](https://www.powershellgallery.com/packages/SolidFire.Core/12.3.0.81). The reason is there's less concern when integrating with Linux iSCSI code and Ansible (PowerShell 7.1.3 on Ubuntu 18.04 is 90% "there", but has some quirks).
...
![solidsync in action](/assets/images/revisiting-solidbackup-00.gif)

Even if the rest of the steps remain unfinished (manual or semi-manual), I think this is a big improvement over SolidFire's built-in [Backup to S3](/2021/04/21/solidfire-backup-to-s3) feature and can complement commercial data protection software.
...
- it's already clear to me that some SolidFire customers are interested in using and automating some steps from SolidBackup workflow, and with small changes we should be able to reuse that to automate SolidFire backups in Kubernetes environments (Velero [works with Restic](https://www.jamesmcleod.xyz/2020/12/09/velero-restic/))
...
![PowerShell wrapper for Trident](/assets/images/manage-netapp-trident-with-powershell-01.png)
...
![SolidSync Demo](/assets/images/solidbackup-01-prepare.gif)
...
![SolidSync Result with cloned volumes belonging to another Account and QoS policy](/assets/images/solidsync-src-to-dst-w-qos-and-account-adjustment.png)
...
![SolidBackup Demo](/assets/images/solidbackup-02-execute.gif)
...
The source code has been [posted](https://github.com/scaleoutsean/solidbackup) to Github.

## Demo

- SolidSync & SolidBackup (Restic version) [demo](https://youtu.be/y7cFBPqdN7s) - 7m30s
